...
Repo-level skills are placed in the standard location under `.agent/skills/`.  Agent must load these skills making sure they are readily available. If user requests to list active/available/registered skills, repo skills must also be listed, but clearly separated from other sources.
